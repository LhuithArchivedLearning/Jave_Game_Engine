...
public class Level 
{
	...
	{
		Transform doorTransfor = new Transform();
		
		Door result = new Door(transform, material);
	}
	
	private void addSpecial(int blueValue)
	{
		if(blueValue == 16)
			addDoor();
	}
	...
	private void GenerateLevel()
	{
		...
		for(int i = 0; i < level.getWidth(); i++)
		{
			for(int j = 0; j < level.getHeight(); j++)
			{
				...
				addSpecial((level.getPixel(i, j) & 0x0000FF));
